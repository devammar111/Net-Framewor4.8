...
@Component({
  selector: 'app-test-sub-form',
  templateUrl: './test-sub-form.component.html',
  styles: []
})
export class TestSubFormComponent implements OnInit {
  ...
  collapsed: any = [false, true, true];
    ...
    testsSub: Menu = pBMenus.find(x => x.id == "TestsSub");
  ...
  ngOnDestroy() {
    let that = this;
    that.stateManagementService.save(that.testsSub.id, that.collapsed);
  }
    ...
    get() {
        ...
        that.httpService
            ...
            .subscribe((res: any) => {
              ...
              that.collapsed = that.stateManagementService.set(that.testsSub.id, that.collapsed);
