...
@Component({
  selector: 'app-test-sub-grid',
  templateUrl: './test-sub-grid.component.html',
  styles: []
})
export class TestSubGridComponent implements OnInit {
    gridOptions: any = {};
    queryParams: any = {};
    tests: Menu = pBMenus.find(x => x.id == "Tests");
    testssSub: Menu = pBMenus.find(x => x.id == "TestsSub");
   @ViewChild('vw_TestSubGrid') vw_TestSubGrid: ElementRef;
    ...
    ngOnInit() {
        ...
        that.gridOptions.GridLabel = that.testssSub.grid.routeInfo.label;
        that.dataSource.queryName = that.cryptoService.decryptString(that.queryParams.localName);
        that.dataSource.init(that.testssSub.grid.vwName, that.gridOptions, 'TestSubId', 'desc', 'TestId', that.queryParams.testId ? that.queryParams.testId : '0');
   ...
   private gridReady(params?: any) {
    ...
    navigate(id?: string) {
        var that = this;
        if (id) {
            that.router.navigate(['/' + that.testssSub.grid.routeInfo.route + '/' + that.testssSub.editForm.routeInfo.route], {
                queryParams: {
                    id: id,
            ...
            that.router.navigate(['/' + that.testssSub.grid.routeInfo.route + '/' + that.testssSub.addForm.routeInfo.route], {
                queryParams: {
                    testId: that.queryParams.testId,
                }
            });
   ...
   reload() {
    ...
    setBreadcrumbs() {
        ...
        breadcrumbs.push({
            label: that.testssSub.grid.routeInfo.label,
            url: '',
            params: {}
        });
